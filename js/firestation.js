...
const FireStation = Lazy('FireStation', () => {
  ...
  const {shape,box,cyl,ball,capsule,taper,model,modelOr,flat,glyphs,solid,shade,glow,light,thing}=B;
  ...
  flat(0,0,0,RX*2,RZ*2,col.floor,
    {gloss:.08,mat:'tile',matScale:.42,matAmt:.28,tag:'消防站地面'});
  flat(2.78,.004,-4.65,18.85,19.0,col.concrete,
    {gloss:.09,mat:'tile',matScale:.58,matAmt:.24,tag:'车库地面'});
  flat(0,.006,10.1,24.45,9.6,col.floorL,
    {gloss:.07,mat:'tile',matScale:.34,matAmt:.22,tag:'生活区地面'});
  ...
  function openWheel(wx,wy,wz,tag) {
    ...
    // Straight spokes terminate on the hub and rim centre lines, so both cylinder caps stay buried.
    for(let i=0;i<6;i++) {
      const a=i*Math.PI/3;
      const mx=wx+Math.cos(a)*.20,my=wy+Math.sin(a)*.20;
      cyl(mx,my,wz,.0135,.30,col.steelL,{rz:a,gloss:.50,tag:`${tag}-轮辐`});
    }
  }
  function lockerOutline(x,y,z,w,h,side,tag) {
    const zz=z+side*1.208;
    box(x,y,zz,w-.07,h-.08,.026,col.redD,{hard:true,gloss:.18,tag});
    box(x,y,zz+side*.018,w-.16,h-.17,.018,col.red,{hard:true,gloss:.34,tag});
  ...
  function fireEngine(z,unit,variant=0) {
    ...
    for(let i=0;i<4;i++)
      box(-1.93+i*1.31,2.04,z,1.23,2.00,2.24,body,
        {round:.12,gloss:.34,tag:`${tag}-器材舱体-${i}`});
    ...
    box(4.86,1.48,z,3.48,1.62,2.36,body,{round:.18,gloss:.34,tag});
    ...
    // Roof extension ladder with two rails, eleven rungs, end stops and support saddles. Each
    // cylinder rung ends on a rail axis, hiding its flat caps inside the wider side tubes.
    for(const side of [-1,1])rodX(.05,3.57,z+side*.57,5.28,.052,col.chrome,
      {tag:`${tag}-云梯`});
    for(let x=-2.35;x<=2.45;x+=.48)cyl(x,3.57,z,.016,1.14,col.chrome,
      {rx:Math.PI/2,tag:`${tag}-云梯`});
  ...
  // Shelf bars terminate inside the 70 mm posts; cylinders retain the round silhouette without
  // spending rounded-end geometry that cannot be seen at any joint.
  for(const y of [.35,1.22,2.10,2.98,4.05]) {
    cyl(-9.26,y,-14.35,.0225,2.58,col.steel,{rz:Math.PI/2,tag:hoseTag});
    cyl(-9.26,y,-13.58,.0225,2.58,col.steel,{rz:Math.PI/2,tag:hoseTag});
    for(const x of [-10.52,-9.68,-8.84,-8.00])
      cyl(x,y,-13.96,.016,.77,col.steel,{rx:Math.PI/2,tag:hoseTag});
  }
  for(let i=0;i<8;i++) {
  ...
  function turnoutSet(x,z,index,tag) {
    ...
    for(const s of [-1,1]) {
      ...
      box(x+.12,.51,z+s*.17,.45,.36,.28,col.black,{round:.08,tag});
      box(x+.25,.32,z+s*.17,.42,.12,.31,col.rubber,{round:.06,tag});
    }
  }
  ...
  // TRELLIS.2 replaces only that shell: the authored plinth, stem and rear cradle remain the
  // stable scale/collision context, and a missing or rejected model executes this exact fallback.
  ...
  modelOr('memorial_fire_helmet',9.88,.925,14.42,1,
    {tag:displayTag,gloss:.35},()=>{
